...
## Rendre un widget invisible

`data-size="invisible"` est ce qui rend le widget invisible. Placez-le sur l'élément qui soumet le formulaire :
...
Le défi exécuté en arrière-plan est une propriété de la clé de site, choisie dans le
[portail Prosopo](https://portal.prosopo.io) — voir [Types de CAPTCHA](/fr/basics/captcha-types/). Avec une clé
`frictionless`, la plupart des visiteurs effectuent une preuve de travail invisible et ne voient jamais rien ; un
visiteur jugé à risque se voit présenter un puzzle ou un défi image au moment de la soumission.
