...
| `createReport` | Report a bad actor you have spotted (see [Reporting Bad Actors](#reporting-bad-actors)) |
...
If you spot a bot or abuser on your own system, report it so Prosopo can improve its protection. Use a key with the `createReport` permission.

```bash
curl -X POST https://api.prosopo.io/reports/create \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"ip": "203.0.113.7", "comment": "Credential stuffing on /login", "score": 0.9}'
```

| Field | Description |
|-------|-------------|
| `ip` | The IP address of the bad actor |
| `userAccount` | The user account of the bad actor |
| `comment` | Optional. What you saw, up to 2000 characters |
| `score` | Optional. How sure you are, from `0` (unsure) to `1` (certain) |

Each report needs an `ip` or a `userAccount`. The response is `{ "id": "...", "kind": "structured" }`.

If the body does not match this shape it is still accepted and stored as-is, and the response `kind` is `plaintext` so you can tell it was not understood.
