...
- `type` is `block` (fail the request outright), `restrict` (serve a harder or easier challenge) or `allow`
  (skip the challenge; see [Allow](/en/advanced/access-control-rules/#allow)). A
...
- `conditions` are ANDed. `field` is one of `ip`, `ipMask`, `userId`, `ja4Hash`, `userAgent`, `countryCode`,
  `asn`, `bypassKeyHash`; `operator` must be `equals` — any other operator is dropped silently.
- `bypassKeyHash` is the lowercase hex SHA-256 of a [bypass key](/en/advanced/bypass-keys/). Send the hash, never
  the key: a value that is not a 64-character hex string is rejected. Pair it with `type: "allow"` and a
  `clientId` naming the site the key is for.
