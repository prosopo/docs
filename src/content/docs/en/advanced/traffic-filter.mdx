...
import TrafficFilter from '~/assets/traffic-filter.png';
...
Each filter is its own toggle on the **Traffic Filter** tab of Site Settings:

<Image src={TrafficFilter} alt="prosopo traffic filter settings" style="margin: 20px auto;"/>
...
See the [Server-side Verification](/en/basics/server-side-verification/#ip-address) docs for details.
