...
Compares the IP's abuse score against the configured `abuseScoreThreshold`. If the score exceeds the threshold, the configured `abuseScoreExceedAction` is triggered.

**Use case:** Reject or flag requests originating from IP addresses with a high abuse reputation score.
...
```typescript
{
    enabled: true,                      // Master toggle for IP validation rules
    actions: {
        ...
        abuseScoreExceedAction: "reject", // Block high abuse-score IPs
    },
    distanceThresholdKm: 1000,          // 1000km maximum distance
    abuseScoreThreshold: 0.005,         // Abuse score above which the action triggers
...
- **Abuse Score Threshold**: 0.005
- **Abuse Score Exceed Action**: Reject
