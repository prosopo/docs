...
To enable IP validation checks, you must pass the user's IP address when verifying the CAPTCHA response. See the [Server-side Verification](/en/basics/server-side-verification/#ip-address) documentation for details on including the IP address in your verification request.
