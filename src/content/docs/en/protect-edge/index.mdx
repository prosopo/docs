...
- **No cookie:** the edge returns **401 Unauthorized** with `X-Prosopo-Status: no-session`. The Protect script (loaded on your app shell) intercepts the 401, creates a session, and replays the request.
...
JSON paths never pass through on a cookie-less request — an API endpoint has no HTML shell to bootstrap a session from, so a first-hit is always either a rule-based decision or a hard 401.
