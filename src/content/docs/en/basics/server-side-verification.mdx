...
| `API.AUTHENTICATED_IP_REQUIRED` | The token was issued without a challenge, by an [Allow rule](/en/advanced/access-control-rules/#allow) or a [bypass key](/en/advanced/bypass-keys/), and these tokens can only be verified with the visitor's `ip`. |
| `API.AUTHENTICATED_IP_MISMATCH` | The token was issued without a challenge, and the `ip` you passed is not the address it was issued to. |
