...
To verify a user's response using JavaScript / TypeScript, instantiate the `ProsopoServer` class from `@prosopo/server` and
call its `isVerified` method, passing it the `procaptcha-response` POST data. Types can be imported from `@prosopo/types`.
