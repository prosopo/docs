...
| 400 | `API.INVALID_SITE_KEY` | The `secret` does not correspond to the site key embedded in the `token`. | Confirm the secret key for the site matches the one rendering the widget. A common cause is using a secret from a different site. The check is skipped for [reserved test site keys](/en/basics/test-keys/), which have no secret. |
