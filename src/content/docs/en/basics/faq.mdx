...
This error occurs when the [captcha type](/en/basics/captcha-types) does not match the type configured for your site key.
The default captcha type is `frictionless`. The captcha type is server-driven per site key, so if you wish to use `pow`
or `image` you must set it in your [portal settings](https://portal.prosopo.io).
