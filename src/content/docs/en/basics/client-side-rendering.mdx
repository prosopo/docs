...
Please note, the client side CAPTCHA type must be set to the same value as in the portal, or a more secure version. For example, if your portal CAPTCHA Type is `frictioness`, you may only set the client side CAPTHCHA Type to `frictionless` or `image`. If your portal CAPTCHA Type is `pow`, you may only set the client side CAPTCHA Type to `pow`.
