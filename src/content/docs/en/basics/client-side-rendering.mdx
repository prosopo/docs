...
| size                 | string             | Set to `invisible` to render the widget in [invisible mode](/en/basics/invisible-captcha/).                                                                          | ✗        |
...
The captcha type is server-driven per site key: you select it in the [portal](https://portal.prosopo.io) rather than
passing it as a render option or data attribute. The available types are:
...
**Please note**, the [CAPTCHA type](/en/basics/captcha-types/) is determined entirely by your site key's configuration in the [portal](https://portal.prosopo.io). There is no client-side option to override it.
